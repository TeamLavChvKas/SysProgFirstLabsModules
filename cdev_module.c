...
static ssize_t my_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
	char charArr[BUF_LEN] = "";
	int var;
	print_msg("Driver Read Function Called...!!!\n");

	// otrimannya vipadkovogo naboru bit (4 bayta)
	get_random_bytes(&var, sizeof(var));
	snprintf(charArr, BUF_LEN, "randomed: %d", var);

	if (len > BUF_LEN) {
		len = BUF_LEN;
	}

	// функция для безопасного копиования данных из области ядра в область пользователя
	if (copy_to_user(buf, charArr, len)) {
		return -EFAULT;
	}
